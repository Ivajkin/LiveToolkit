...
БД напоминает солнышко -- в центре находится таблица Elements, в который все элементы являются внешними ключами. И её "лучи" в виде связей направлены к ключевым полям других таблиц. Всей этой идилии мешает пара связей между Lessons и Books, Lessons и Interactive Exercises. Эти свчзи показывает зависимость интерактивных упражнений и книг от уроков.
...
Остальные таблицы в БД разделены на три группы в по степени свободы (или тепени зависимости) относительно таблицы уроков:
...
Также в основном модуле необходима своя БД. Назовём её _*platformBD*_. О принципе действия такой структуры поговорим далее.
...
Возможна ещё такая ситуация: мы выпуcтили модуль, в котором содержится только урок. Через некоторое время мы хотим дополнить его упражнениями и другим материалом. В этом случае в поле Data не пишется заново контекст урока, а пишется например _*UPDATE*_ -- и урок из одного модуля дополняется упражнениями из друго. Впрочем обновление материала следует продумать отдельно.
...
При установке модуля, в папку *Lessons\lesson_name* копируется приведенная на рисунке БД -- она выстумает как хранилище уроков, а в папку *Lessons\lesson_name* -- рисунки на изображения в уроках и упражнениях.
####Books
...
Определение новых слов, планируется использовать по html-тегам в тексте книги. Не вижу надобности здесь использовать поле Param. Однако если но понадобится, добавить его не составит труда.
...
1. Data -- на первой строчке тег *GAME* для идентификации типа приложения. При установке модуля, игры копируются в папку *interactive*
2. Data -- на первой строчке тег *UPDATE*. Зпускается при завершении установки модуля -- когда все другие елементы обработаны -- и выполняет обновление.
3. Data -- на первой строчке тег *BANNER*. При установке модуля копируется куда-нибудь откуда не догадаешься удалить.
