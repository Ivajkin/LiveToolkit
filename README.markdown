#"Редактор Live"
Админская панель (веб) для редактирования сущностей предметной области в базе данных.
Предназначение **редактора** - автоматизация и упрощение редактирования БД, сохраняя её целостность, поддерживая её соответствие внешним функциональным требованиям и органичениям.
...
2. Книги -- текст со вставками, при нажатии на которые можно посмотреть значение слова и послушать его произношение. Нажимаемые добавляется в общий словарь пользователя, отображаются справа. 
3. *Интерактивные упражнения*, тесты -- выбор слов по их звучанию (с картинками), ответы на вопросы, соотнесение слов их значению, рисование иероглифа
4. Видео на китайском с Youtube с интерактивными субтитрами. При нажатии на иероглиф в субтитрах, иероглиф добавляется в словарь неизвестных слов пользователя, этот словарь отображается справа, новые слова вверху - иероглиф, произношение, перевод, кнопка для проигрывания звука (в дальнейшем эта информация будет включена в общую статистику по изучению иероглифов пользователя для использования в частоте встрече иероглифа в упражнениях).
5. Аудирование -- диалоги, песни, и т.п. на изучаемом языке и в виде аудиофайлов. Принцип тот же, что и в **видео**.
...
В каждом разделе редактора нужен элемент для просмотра. При нажаии на кнопку !["Просмотр"](http://cdn1.iconfinder.com/data/icons/WPZOOM_Developer_Icon_Set/32/eye.png) (кнопка с глазом), открывается отдельное окно с работающим упражнением.
Всё что редактируется должно при бекапе и в осстановлении полностью вернуться к состоянию на момент бекапа.
Изменения в систему накатываем с помощью скриптов, автоматически: [Continuous integration](http://en.wikipedia.org/wiki/Continuous_integration)
...
##Эпилог
Поздравляю всех кто дочитал до конца! Вы сделали это, вы на себе испытали мощь LiveSDK! Был бы рад услышать как эту систему можно упростить в 10 раз без потери мощности и гибкости инструментария. Давайте поднимем эту софтину!
